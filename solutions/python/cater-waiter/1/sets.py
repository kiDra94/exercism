...
def tag_special_ingredients(dish):
    ...
    """
    return tuple((dish[0], set(dish[1]).intersection(SPECIAL_INGREDIENTS)))
...
def compile_ingredients(dishes):
    ...
    """
    unioned = set()
    for ingridents in dishes:
        unioned = unioned.union(ingridents)
    return unioned
...
def separate_appetizers(dishes, appetizers):
    ...
    dishes = dishes.difference(appetizers)
    return list(dishes)
...
def singleton_ingredients(dishes, intersection):
    ...
    for dish in dishes:
        all.update(dish)
    singelton = all.difference(intersection)
    result = set()
    for ingridient in singelton:
        count = sum(1 for dish in dishes if ingridient in dish)
        if count == 1:
